...
(progn 
  ...
  #+ccl-2                              ; This isn't Coral specific
     (defmacro with-augmented-environment
	      ((new-env old-env &key functions macros) &body body)
      `(let ((,new-env (with-augmented-environment-internal ,old-env
                         ,functions
...
(pushnew :allegro-v4.1+ *features*)

(defmacro special-form-p (sym) `(cltl1:special-form-p ,sym))
...
(defun with-augmented-environment-internal (env functions macros)
  (let (#+allegro-v4.1+ (env-tail (cdr env)) #+allegro-v4.1+ (env (car env)))
    ...
    #-allegro-v4.1+ env
    #+allegro-v4.1+ (cons env env-tail)))
...
;; FIXTHIS The problem seems to be around here.  the type of contour
;; being created by this version of PCL doesn't match
